...
void ARigidBody2DSync::set_physics_state()
{
    ...
    if (parent->get_position() != last_pos && NETWORK_MANAGER->GAME_STARTED) 
    {
        Dictionary DATA = Dictionary();
        DATA["I"] = packet_index + 1;
        DATA["T"] = ANetworkManager::SEND_TYPE::RIGIDBODY_SYNC;
        Array Value = Array();
        Value[0] = parent->get_linear_velocity();
        Value[1] = parent->get_angular_velocity();
        Value[2] = parent->get_position();
        Value[3] = parent->get_rotation();
        DATA["V"] = Value;
        DATA["NP"] = get_path();
        P2P->_send_P2P_Packet(0,0,DATA,Steam::P2PSend::P2P_SEND_UNRELIABLE);
        packet_index = packet_index + 1;
        last_pos = parent->get_position();
    }
...
void ARigidBody2DSync::update_physics_state()
{
    ...
    parent->set_position(UtilityFunctions::lerp(parent->get_position(), target_pos, interpolation_value));
